...
action :install do
  ...
  template "/etc/haproxy/haproxy.cfg.head" do
    ...
    variables(
      ...
      :timeout_client => node[:lb_haproxy_multiapp][:timeout_client],
      :global_maxconn => node[:lb_haproxy_multiapp][:global_maxconn],
      :default_maxconn => node[:lb_haproxy_multiapp][:default_maxconn],
      :httpclose => node[:lb_haproxy_multiapp][:httpclose],
      :abortonclose => node[:lb_haproxy_multiapp][:abortonclose]
    )
  end
...
action :add_vhost do
  ...
  # See cookbooks/lb_haproxy_multiapp/definitions/haproxy_backend.rb for the definition
  # of "lb_haproxy_multiapp_backend".
  lb_haproxy_multiapp_backend "create main backend section" do
    pool_name pool_name
  end
