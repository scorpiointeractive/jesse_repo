...
directory node[:web_apache_multiapp][:docroot] do
  recursive true
end
...
#if node[:web_apache_multiapp][:ssl_enable]
#  include_recipe "apache2::mod_ssl"
#end

# force install ssl module
include_recipe "apache2::mod_ssl"
...
when "centos", "redhat"
  ...
  binary_to_use << ".#{node[:web_apache_multiapp][:mpm]}" unless node[:web_apache_multiapp][:mpm] == 'prefork'
...
when "ubuntu"
  package "apache2-mpm-#{node[:web_apache_multiapp][:mpm]}"
end
...
template File.join(node[:apache][:dir], 'conf.d', 'maintenance.conf') do
  ...
  variables(
    :maintenance_file => node[:web_apache_multiapp][:maintenance_file]
  )
